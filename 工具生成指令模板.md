...
`tools/xlsx-sheet-summary/tool.ps1`
...
# 执行 officecli 并把 stdout 解析为 JSON
function Get-OCJson {
  param([string[]]$Arguments)
  $raw = & $oc @Arguments 2>$null
  if ($LASTEXITCODE -ne 0) { throw "officecli 命令失败: $($Arguments -join ' ')" }
  $text = ($raw | Out-String).Trim()
  $start = $text.IndexOf('{')
  $end = $text.LastIndexOf('}')
  if ($start -lt 0 -or $end -lt 0) { throw "无法解析 officecli 输出: $($Arguments -join ' ')" }
  return ($text.Substring($start, $end - $start + 1) | ConvertFrom-Json)
}

# 读取整个工作簿
$doc = Get-OCJson @('get', $inputFile, '/', '--json')
$sheets = @($doc.data.results[0].children | Where-Object { $_.type -eq 'sheet' })
if ($sheets.Count -eq 0) {
  Write-Error "文件中没有工作表: $inputFile"
  exit 1
}

$summaries = @()
foreach ($s in $sheets) {
  Write-Output ('{"progress":"统计工作表 ' + $s.preview + '"}')
  $rows = @($s.children)
  $maxCols = 0
  foreach ($r in $rows) {
    $cols = @($r.children).Count
    if ($cols -gt $maxCols) { $maxCols = $cols }
  }
  $summaries += @{ name = $s.preview; rows = $rows.Count; columns = $maxCols }
}

# 结果 JSON 必须是 stdout 最后一行
$result = @{
  ok          = $true
  summary     = "已统计 $($sheets.Count) 个工作表"
  outputFiles = @()   # 无输出文件
  sheets      = $summaries
...
```
按 Doc-Helper 工具规范生成工具，产出 meta.json + tool.ps1 两个文件：
...
3. tool.ps1：第一行 param() 块（参数名与 meta.json 对齐），其后设 OutputEncoding=UTF8；
   UTF-8 BOM 保存；经 $env:OFFICECLI 调 officecli（不确定就 officecli help 查询）；
   成功时 stdout 最后一行 {"ok":true,"outputFiles":[...],"summary":"..."}；
   失败 Write-Error + exit 1；可选 {"progress":"..."} 进度行
4. 自检：参数对齐、UTF-8、OFFICECLI、结果 JSON 最后一行、边界校验
```
...
- 参数：`type:"file"` 的实参由后端映射为上传目录（`@uploads`）；`output:true` 前端识别为输出路径
- 执行：`powershell -NoProfile -ExecutionPolicy Bypass -File tool.ps1 -key value ...`，注入 `OFFICECLI` 环境变量，默认超时 120s，可取消
- 成功解析：从 stdout 末尾向前找最后一个可解析 JSON 对象；`ok===false` 时以后端 result.error 报错
